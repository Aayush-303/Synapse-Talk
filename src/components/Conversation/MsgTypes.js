    ...
    <Stack direction = "row" justifyContent={el.incoming ? "start" : "end" }>
        <Box p = {1.5} sx = {{backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content",}}>
            <Stack spacing={2}>
                ...
                <Typography variant='body2' sx = {{color: el.incoming ? theme.palette.text : "#ffffff"}}>{el.message}</Typography>
            </Stack>  
        </Box>
    </Stack>
  )
...
const LinkMsg = ({el}) => {
  ...
  return (
    <Stack direction = "row" justifyContent={el.incoming ? "start" : "end" }>
        <Box p = {1.5} sx = {{backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content",}}>
            <Stack spacing={2}>
                <Stack p = {2} spacing = {3} alignItems = "start" directtion = "column" sx = {{backgroundColor: theme.palette.background.paper, borderRadius: 1}}>
                    ...
                    <Typography variant='body2' color = {el.incoming ? theme.palette.text : "#ffffff"}>
                        {el.message}
                    </Typography>
                </Stack>
            </Stack>   
...
const ReplyMsg = ({el}) => {
  ...
  return (
    <Stack direction = "row" justifyContent={el.incoming ? "start" : "end"}>
        <Box p = {1.5} sx = {{backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content",}}>
            <Stack spacing = {2}>
                <Stack p = {2} dorection = "column" spacing = {3} alignItems={"center"} sx = {{backgroundColor: theme.palette.background.paper, borderRadius: 1}}>
